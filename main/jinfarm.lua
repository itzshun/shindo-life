...
while getgenv().Jin.AutoEquip do wait()
    if getgenv().Jin.AutoEquip == true then
        ...
        elseif not FindBijuu and FindMode.Value >= getgenv().Jin.MaxMD then
            ...
            local A_1 = "key"
            local A_2 = KeyState
            local Event = game:GetService("Player").LocalPlayer.combat.update
            Event:FireServer(A_1, A_2)
        end
        local FindChakra = game:GetService("Players").LocalPlayer.statz.mastery.chakra
        if FindChakra.Value == getgenv().Jin.Chakra or FindChakra.Value >= getgenv().Jin.Chakra or FindChakra.Value <= getgenv().Jin.Chakra then
            print("Wait")
        else
            local A_1 = "addstat"
            local A_2 = "chakra"
        ...
        if FindNin.Value == getgenv().Jin.Ninjutsu or FindNin.Value >= getgenv().Jin.Ninjutsu or FindNin.Value <= getgenv().Jin.Ninjutsu then
            print("Wait NIn")
        else
            local A_1 = "addstat"
            local A_2 = "ninjutsu"
